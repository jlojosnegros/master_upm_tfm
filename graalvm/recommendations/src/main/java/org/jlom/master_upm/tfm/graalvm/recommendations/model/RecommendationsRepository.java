package org.jlom.master_upm.tfm.graalvm.recommendations.model;
...
public class RecommendationsRepository implements IRecommendationsRepository {
  ...
  public void save(String userId, WeightedTag tag) throws IOException {
    ...
    redisApi.set(userCollectionKey, JsonUtils.ObjectToJson(tag));
  ...
  public boolean update(String userId, String tagName, long delta) {
    ...
    try {
      WeightedTag weightedTag = JsonUtils.jsonToObject(jsonData, WeightedTag.class);
      weightedTag.setWeight(weightedTag.getWeight() + delta);
      redisApi.set(userCollectionKey, JsonUtils.ObjectToJson(weightedTag));
  ...
  public void delete(String userId) {
    ...
    List<String> tagNamesForUser = redisApi.keys(userCollectionKey)
            ...
            .map(jsonData -> {
              try {
                return JsonUtils.jsonToObject(jsonData, WeightedTag.class);
              } catch (IOException e) {
                e.printStackTrace();
                return null;
              }
